...
type UseHasuraQueryFunction<T = any> = (
  args: UseHasuraQueryArgs
) => QueryResult<T>;

const useHasuraQuery: UseHasuraQueryFunction = ({
  fields,
  operation,
  ...
  variables,
  where
}: UseHasuraQueryArgs): QueryResult => {
  ...
  useEffect(() => {
    if (result.data && schema) {
      const camelCaseData = camelCaseKeys(result.data[operation], {
        deep: true
      });
